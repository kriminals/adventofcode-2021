...
let fourDigits = displays.split('\n').map(entry => entry.split('|')[1].trimStart()).flatMap(digits => digits.split(' '))
let uniqueDigits = fourDigits.reduce((acc, digit) => {
    if ((digit.length == 2) || (digit.length == 3) || (digit.length == 4) || (digit.length == 7)) return ++acc
    return acc
}, 0)
