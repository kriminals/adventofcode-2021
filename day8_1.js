let displays = `bgeacd dbfag bcadegf agdce dgfbce bgc bdgca aedcgf bc abec | gcdfbe cbea bc gbc
...
let tenDigits = displays.split('\n').map(entry => entry.split('|')[0].trimEnd()).map(digits => digits.split(' '))
let fourDigits = displays.split('\n').map(entry => entry.split('|')[1].trimStart()).map(digits => digits.split(' '))
let decodedTenDigits = tenDigits.map(entry => decode(entry))

// digits is a table of 10 strings
function decode(digits) {
    let one = digits.find(entry => entry.length == 2)
    let four = digits.find(entry => entry.length == 4)
    let seven = digits.find(entry => entry.length == 3)
    let eight = digits.find(entry => entry.length == 7)
    let six_nine_zero = digits.filter(entry => entry.length == 6)
    let sixIndex = six_nine_zero.map(entry => entry.replace(one[0],'').replace(one[1],'').trim()).findIndex(entry => entry.length == 5)
    let six = six_nine_zero[sixIndex]
    let two_three_five = digits.filter(entry => entry.length == 5)
    let threeIndex = two_three_five.map(entry => entry.replace(one[0],'').replace(one[1],'').trim()).findIndex(entry => entry.length == 3)
    let three = two_three_five[threeIndex]
    let two_five = two_three_five.filter(entry => entry !== three)
    let fiveIndex = two_five.map(entry => entry.replace(four[0], '').replace(four[1], '').replace(four[2], '').replace(four[3], '').trim()).findIndex(entry => entry.length == 2)
    let twoIndex = two_five.map(entry => entry.replace(four[0], '').replace(four[1], '').replace(four[2], '').replace(four[3], '').trim()).findIndex(entry => entry.length == 3)
    let five = two_five[fiveIndex]
    let two = two_five[twoIndex]
    let nine_zero = six_nine_zero.filter(entry => entry !== six)
    let nineIndex = nine_zero.map(entry => entry.replace(three[0], '').replace(three[1], '').replace(three[2], '').replace(three[3], '').replace(three[4], '').trim()).findIndex(entry => entry.length == 1)
    let zeroIndex = nine_zero.map(entry => entry.replace(three[0], '').replace(three[1], '').replace(three[2], '').replace(three[3], '').replace(three[4], '').trim()).findIndex(entry => entry.length == 2)
    let nine = nine_zero[nineIndex]
    let zero = nine_zero[zeroIndex]
    return [
        zero.split('').sort().join(''),
        one.split('').sort().join(''),
        two.split('').sort().join(''),
        three.split('').sort().join(''),
        four.split('').sort().join(''),
        five.split('').sort().join(''),
        six.split('').sort().join(''),
        seven.split('').sort().join(''),
        eight.split('').sort().join(''),
        nine.split('').sort().join('')
    ]
} 
let output = fourDigits.reduce((acc, digits, index) => {
    let sortedFourDigits = digits.map(entry => entry.split('').sort().join(''))
    let intFourDigits = parseInt(sortedFourDigits.flatMap(entry => decodedTenDigits[index].indexOf(entry)).join(''))
    return acc + intFourDigits
